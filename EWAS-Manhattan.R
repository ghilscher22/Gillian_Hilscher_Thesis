...
# Plot the beta data as a line plot with three different colors
pdf("Line Beta Value for Shepherd CpGs in Transmasculine Youth.pdf")
ggplot(data = beta_allTBshep_meansd, 
       aes(x = rownames(beta_allTBshep_meansd), 
           group = 1)) + 
    geom_point(aes(y = mean_beta1, color = "0 Months (Baseline)"), size = 1.5) + 
    geom_point(aes(y = mean_beta2, color = "3-5 Months"), size = 1.5) + 
    geom_point(aes(y = mean_beta3, color = "12 Months"), size = 1.5) + 
    geom_line(aes(y = mean_beta1, color = "0 Months (Baseline)")) + 
    geom_line(aes(y = mean_beta2, color = "3-5 Months")) + 
    geom_line(aes(y = mean_beta3, color = "12 Months")) + 
    geom_errorbar(aes(ymin = mean_beta1 - sd_beta1, 
                      ymax = mean_beta1 + sd_beta1), width = 0.1,
                  color="black",
                  size=0.25) +
    geom_errorbar(aes(ymin = mean_beta2 - sd_beta2, 
                      ymax = mean_beta2 + sd_beta2), width = 0.1,
                  color="black",
                  size=0.25) +
    geom_errorbar(aes(ymin = mean_beta3 - sd_beta3, 
                      ymax = mean_beta3 + sd_beta3), width = 0.1,
                  color="black",
                  size=0.25) +
    ...
    labs(x = "CpG Site",
         y = "Beta Value",
         title = "Beta Values for Shepherd (2022) CpGs Over Time",
         size = "",
         colour = "Time on Testosterone") +
    scale_x_discrete(limits = rownames(top_5col_shepherdTB)) +
    scale_color_manual(values = c("darkolivegreen2",
                                    "darkolivegreen3",
                                    "darkolivegreen"),
                        breaks=c('0 Months (Baseline)', 
                                 '3-5 Months', 
                                 '12 Months'))
dev.off()